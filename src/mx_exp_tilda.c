...
static void tilda_user(int i, char *line, char **head) {
    ...
    pw = getpwnam(buf + 1);
    if (pw) {
        *head = mx_sub_replace(line, buf, pw->pw_dir);
    }
}
...
static void tilda(char **head, char *line, char flag) {
    if (flag == 'd') {
        if (!getenv("HOME")) {
            ...
            *head = mx_sub_replace(line, "~", buf->pw_dir);
            }
        else
            *head = mx_sub_replace(line, "~", strdup(getenv("HOME")));
    }
    if (flag == '+')
        *head = mx_sub_replace(line, "~+", strdup(getenv("PWD")));
    if (flag == '-')
        *head = mx_sub_replace(line, "~-", strdup(getenv("OLDPWD")));
}
